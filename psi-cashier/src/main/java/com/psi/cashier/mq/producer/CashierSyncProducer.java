...
import com.psi.cashier.feign.SyncUpFeignClient;
...
/**
 * 收银微服务数据同步生产者
 * 使用虚拟线程异步调用数据库，组装数据后通过Feign直接上传到sync模块
 * 替代原MQ方式，便于单体部署
 */
@Slf4j
@Component
...
public class CashierSyncProducer {
    ...
    private final SyncUpFeignClient syncUpFeignClient;
    ...
    /**
     * 发送同步数据到sync模块（通过Feign直接调用）
     */
    private void sendSyncData(SyncBatchDTO batchDTO) {
        try {
            CommonResult<Boolean> result = syncUpFeignClient.batchUpload(batchDTO);
            if (result != null && result.isSuccess()) {
                log.debug("同步数据上传成功: batchUuid={}, dataCount={}",
                        batchDTO.getBatchUuid(), batchDTO.getDataList().size());
            } else {
                log.warn("同步数据上传失败: batchUuid={}, result={}",
                        batchDTO.getBatchUuid(), result);
            }
        } catch (Exception e) {
            log.error("同步数据上传异常: batchUuid={}", batchDTO.getBatchUuid(), e);
        }
    }

    /**
     * 异步发送同步数据到sync模块（使用虚拟线程，不阻塞主流程）
     */
    private void asyncSendSyncData(SyncBatchDTO batchDTO) {
        VirtualThreadContextWrapper.executeAsync(() -> {
            try {
                sendSyncData(batchDTO);
            } catch (Exception e) {
                log.error("异步同步数据上传失败: batchUuid={}", batchDTO.getBatchUuid(), e);
            }
        });
    }
}