...
public class SqliteInitConfig {
    ...
    /**
     * 同步预初始化数据库：如果数据库中已有业务表但 flyway_schema_history 为空，
     * 说明是复制过来的预初始化数据库，将所有迁移脚本标记为已执行，避免重复建表失败。
     */
    private void syncPreInitializedDatabase() throws Exception {
        try (Connection conn = dataSource.getConnection()) {
            // 检查是否存在关键业务表（如 order_item）
            boolean hasBusinessTable = false;
            try (PreparedStatement ps = conn.prepareStatement(
                    "SELECT name FROM sqlite_master WHERE type='table' AND name='order_item'")) {
                try (ResultSet rs = ps.executeQuery()) {
                    hasBusinessTable = rs.next();
                }
            }

            if (!hasBusinessTable) {
                return; // 不是预初始化数据库，正常走 migration 流程
            }

            // 检查 flyway_schema_history 是否已有记录
            ...
            if (hasHistory) {
                return; // 已有记录，正常走 migration 流程
            }

            log.info("检测到预初始化数据库，同步迁移记录...");

            // 收集所有 migration 脚本版本并排序
            ...
            sortedResources.sort(Comparator.comparingInt(r -> {
            ...
            log.info("已同步 {} 条迁移记录", sortedResources.size());
        }
    }
    ...
    private void executePendingMigrations() throws Exception {
        ...
        // 收集并排序迁移脚本（按版本号数字排序，避免 V10 排在 V2 前面）
        List<Resource> sortedResources = new ArrayList<>(List.of(resources));
        sortedResources.sort(Comparator.comparingInt(r -> {
            String filename = r.getFilename();
            if (filename == null) return 0;
