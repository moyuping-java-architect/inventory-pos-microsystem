...
public class StockServiceImpl extends ServiceImpl<StockMapper, StockEntity> implements StockService {
    ...
    public PageResult<StockDTO> list(StockQueryDTO queryDTO) {
        ...
        List<StockEntity> records = result.getRecords();

        // 补全 goodsName / goodsSpec / unit （跨库查 goods 表）
        if (records != null && !records.isEmpty()) {
            List<String> goodsCodes = records.stream()
                    .map(StockEntity::getGoodsCode)
                    .filter(code -> code != null && !code.isEmpty())
                    .distinct()
                    .collect(Collectors.toList());
            if (!goodsCodes.isEmpty()) {
                try {
                    List<Map<String, Object>> goodsInfoList = baseMapper.selectGoodsInfoByCodes(goodsCodes);
                    if (goodsInfoList != null && !goodsInfoList.isEmpty()) {
                        Map<String, Map<String, Object>> goodsInfoMap = goodsInfoList.stream()
                                .collect(Collectors.toMap(
                                        m -> String.valueOf(m.get("goodsCode")),
                                        m -> m,
                                        (a, b) -> a
                                ));
                        for (StockEntity entity : records) {
                            Map<String, Object> info = goodsInfoMap.get(entity.getGoodsCode());
                            if (info != null) {
                                if ((entity.getGoodsName() == null || entity.getGoodsName().isEmpty())
                                        && info.get("goodsName") != null) {
                                    entity.setGoodsName(String.valueOf(info.get("goodsName")));
                                }
                                if ((entity.getGoodsSpec() == null || entity.getGoodsSpec().isEmpty())
                                        && info.get("goodsSpec") != null) {
                                    entity.setGoodsSpec(String.valueOf(info.get("goodsSpec")));
                                }
                                if ((entity.getUnit() == null || entity.getUnit().isEmpty())
                                        && info.get("unit") != null) {
                                    entity.setUnit(String.valueOf(info.get("unit")));
                                }
                            }
                        }
                    }
                } catch (Exception e) {
                    log.warn("补全库存商品信息失败: {}", e.getMessage());
                }
            }
        }
    ...
    /**
     * 根据 goodsCode 查询商品表，补全库存实体的 goodsName / goodsSpec / unit 冗余字段
     */
    private void fillGoodsInfo(StockEntity entity, String goodsCode) {
        if (goodsCode == null || goodsCode.isEmpty()) {
            return;
        }
        try {
            List<Map<String, Object>> list = baseMapper.selectGoodsInfoByCodes(java.util.Collections.singletonList(goodsCode));
            if (list != null && !list.isEmpty()) {
                Map<String, Object> info = list.get(0);
                if (info.get("goodsName") != null) {
                    entity.setGoodsName(String.valueOf(info.get("goodsName")));
                }
                if (info.get("goodsSpec") != null) {
                    entity.setGoodsSpec(String.valueOf(info.get("goodsSpec")));
                }
            ...
            log.warn("补全库存商品信息失败, goodsCode={}: {}", goodsCode, e.getMessage());
        }
    }
    ...
    public CommonResult<Void> increaseStock(String warehouseCode, String goodsCode, String skuCode, BigDecimal quantity,
        ...
        return increaseStock(warehouseCode, goodsCode, skuCode, quantity, costPrice, sourceNo, sourceType, null, null, null);
    }
    ...
    public CommonResult<Void> increaseStock(String warehouseCode, String goodsCode, String skuCode, BigDecimal quantity,
                                             BigDecimal costPrice, String sourceNo, String sourceType,
                                             String goodsName, String goodsSpec, String unit) {
        ...
        if (entity == null) {
            ...
            // 仍有空字段时跨库查询补全
            if (entity.getGoodsName() == null || entity.getGoodsName().isEmpty()
                    || entity.getGoodsSpec() == null || entity.getGoodsSpec().isEmpty()
                    || entity.getUnit() == null || entity.getUnit().isEmpty()) {
                fillGoodsInfo(entity, goodsCode);
            }
            super.save(entity);
        } else {
            ...
            if ((entity.getGoodsName() == null || entity.getGoodsName().isEmpty())
                    || (entity.getGoodsSpec() == null || entity.getGoodsSpec().isEmpty())
                    || (entity.getUnit() == null || entity.getUnit().isEmpty())) {
                if (goodsName != null && !goodsName.isEmpty() && (entity.getGoodsName() == null || entity.getGoodsName().isEmpty())) {
                    entity.setGoodsName(goodsName);
                }
                if (goodsSpec != null && !goodsSpec.isEmpty() && (entity.getGoodsSpec() == null || entity.getGoodsSpec().isEmpty())) {
                    entity.setGoodsSpec(goodsSpec);
                }
                if (unit != null && !unit.isEmpty() && (entity.getUnit() == null || entity.getUnit().isEmpty())) {
                    entity.setUnit(unit);
                }
                if (entity.getGoodsName() == null || entity.getGoodsName().isEmpty()
                        || entity.getGoodsSpec() == null || entity.getGoodsSpec().isEmpty()
