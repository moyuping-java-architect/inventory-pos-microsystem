    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain filterChain)
        ...
        if (StringUtils.hasText(authorizationHeader) && authorizationHeader.startsWith("Bearer ")) {
            ...
            try {
                ...
                // Security fix: Always use token values, never trust client-side headers.
                // The JWT token is signed by the server and contains all user info (tenantId,
                // shopId, warehouseId, userId, roleId, roleCode, roleName, permissions).
                // Reading these from request headers allows permission forgery — a user can
                // modify localStorage or craft headers to escalate privileges.
