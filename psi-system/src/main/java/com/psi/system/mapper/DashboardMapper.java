...
public interface DashboardMapper {
     ...
     * 今日采购金额
     */
    @Select("SELECT COALESCE(SUM(total_amount), 0) FROM purchase_in_main WHERE in_date = #{today} AND del_flag = 0 AND status = 1")
    BigDecimal selectTodayPurchaseAmount(@Param("today") String today);

    /**
     * 今日销售金额
     */
    @Select("SELECT COALESCE(SUM(total_amount), 0) FROM sale_out_main WHERE out_date = #{today} AND del_flag = 0 AND status = 1")
    BigDecimal selectTodaySaleAmount(@Param("today") String today);

    /**
     * 本月采购金额
     */
    @Select("SELECT COALESCE(SUM(total_amount), 0) FROM purchase_in_main WHERE in_date LIKE CONCAT(#{month}, '%') AND del_flag = 0 AND status = 1")
    BigDecimal selectMonthPurchaseAmount(@Param("month") String month);

    /**
     * 本月销售金额
     */
    @Select("SELECT COALESCE(SUM(total_amount), 0) FROM sale_out_main WHERE out_date LIKE CONCAT(#{month}, '%') AND del_flag = 0 AND status = 1")
    BigDecimal selectMonthSaleAmount(@Param("month") String month);

    /**
     * 库存 SKU 数量
     */
    @Select("SELECT COUNT(DISTINCT sku_code) FROM stock WHERE del_flag = 0")
    ...
    @Select("<script>" +
            ...
            "FROM sale_out_main WHERE out_date IN " +
    ...
    @Select("SELECT s.goods_name AS name, s.sku_code AS code, s.quantity AS stock, g.min_stock_qty AS minStock " +
            "FROM stock s LEFT JOIN goods_sku g ON s.sku_code = g.sku_code " +
    ...
    @Select("SELECT orderNo, type, customer, amount, status, createTime FROM (" +
            ...
            "FROM purchase_in_main WHERE del_flag = 0 AND status = 1 " +
            "UNION ALL " +
            ...
            "FROM sale_out_main WHERE del_flag = 0 AND status = 1 " +
