...
public class MemberInfoServiceImpl implements MemberInfoService {
    ...
    public CommonResult<Void> add(MemberInfoDTO dto) {
        ...
        eventPublisher.publish(
                BusinessEvent.of("MEMBER.REGISTERED", BusinessEvent.SUBJECT_MEMBER, member.getId())
                        .bizKey(member.getMemberNo())
                        .put("memberCode", member.getMemberNo())
    ...
    public CommonResult<Void> update(MemberInfoDTO dto) {
        ...
        // 编辑会员时改了等级，同样算一次等级变动，走与自动升级相同的事件
        if (newLevelName != null && !newLevelName.equals(oldLevelName)) {
            eventPublisher.publish(
                    BusinessEvent.of("MEMBER.LEVEL_UPGRADED", BusinessEvent.SUBJECT_MEMBER, member.getId())
                            .bizKey(member.getMemberNo() + ":" + newLevelName)
                            .put("oldLevel", oldLevelName)
                            .put("newLevel", newLevelName)
                            .put("upgradeReason", "MANUAL_EDIT"));
        }
    ...
    public CommonResult<Void> recharge(Long memberId, BigDecimal amount, String sourceNo) {
        ...
        eventPublisher.publish(
                BusinessEvent.of("MEMBER.RECHARGED", BusinessEvent.SUBJECT_MEMBER, memberId)
                        .bizKey(sourceNo)
                        .put("rechargeAmount", amount)
                        .put("balanceAfter", afterBalance));
    ...
    public CommonResult<Void> consume(Long memberId, BigDecimal amount, Integer points, String sourceNo) {
        ...
        eventPublisher.publish(
                BusinessEvent.of("MEMBER.CARD_CONSUMED", BusinessEvent.SUBJECT_MEMBER, memberId)
                        .bizKey(sourceNo)
                        .put("consumeAmount", amount)
                        .put("balanceAfter", afterBalance));

        if (earnPoints > 0) {
            eventPublisher.publish(
                    BusinessEvent.of("MEMBER.POINTS_CHANGED", BusinessEvent.SUBJECT_MEMBER, memberId)
                            .bizKey(sourceNo + ":EARN")
                            .put("pointsChange", earnPoints)
                            .put("pointsAfter", afterPoints)
                            .put("reason", "CONSUME_EARN"));
        }
    ...
    public CommonResult<Void> addPoints(Long memberId, Integer points, String sourceNo, String remark) {
        ...
        eventPublisher.publish(
                BusinessEvent.of("MEMBER.POINTS_CHANGED", BusinessEvent.SUBJECT_MEMBER, memberId)
                        .bizKey(sourceNo != null ? sourceNo + ":ADD" : null)
                        .put("pointsChange", points)
                        .put("pointsAfter", afterPoints)
                        .put("reason", "MANUAL_ADD"));
    ...
    public CommonResult<Void> deductPoints(Long memberId, Integer points, String sourceNo, String remark) {
        ...
        eventPublisher.publish(
                BusinessEvent.of("MEMBER.POINTS_CHANGED", BusinessEvent.SUBJECT_MEMBER, memberId)
                        .bizKey(sourceNo != null ? sourceNo + ":DEDUCT" : null)
                        .put("pointsChange", -points)
                        .put("pointsAfter", afterPoints)
                        .put("reason", "MANUAL_DEDUCT"));
    ...
    public CommonResult<Void> upgradeLevel(Long memberId, Long levelId) {
        ...
        eventPublisher.publish(
                BusinessEvent.of("MEMBER.LEVEL_UPGRADED", BusinessEvent.SUBJECT_MEMBER, memberId)
                        .bizKey(member.getMemberNo() + ":" + level.getLevelName())
                        .put("oldLevel", oldLevelName)
                        .put("newLevel", level.getLevelName())
                        .put("upgradeReason", "MANUAL_UPGRADE"));
