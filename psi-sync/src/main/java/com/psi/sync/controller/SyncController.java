...
public class SyncController {
     ...
     * @return 上传结果
     */
    @PostMapping("/up/batchUpload")
    public CommonResult<Boolean> batchUpload(@RequestBody com.psi.common.dto.sync.SyncBatchDTO batchDTO) {
        log.info("POS批量上传上行同步数据: batchUuid={}, tenantId={}, dataCount={}",
                batchDTO.getBatchUuid(), batchDTO.getTenantId(),
                batchDTO.getDataList() != null ? batchDTO.getDataList().size() : 0);

        if (batchDTO.getDataList() == null || batchDTO.getDataList().isEmpty()) {
            return CommonResult.fail("数据列表为空");
        }

        try {
            String dataListJson = new com.fasterxml.jackson.databind.ObjectMapper()
                    .writeValueAsString(batchDTO.getDataList());
            
            boolean result = upSyncService.batchUpload(
                    batchDTO.getBatchUuid(),
                    batchDTO.getTenantId(),
                    batchDTO.getShopCode(),
                    "cashier-pos",
                    dataListJson
            );
