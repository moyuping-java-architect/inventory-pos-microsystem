 ...
 * 财务上行同步数据消费者
 *
 * <p>监听财务微服务发送的上行同步数据，存储到 up_sync 表等待下游处理。</p>
 * <p>TODO：收银端相关表未创建，目前只保存到 up_sync 中间库，后续按需解析到业务表。</p>
 */
@Slf4j
@Component
public class FinanceSyncConsumer {
    ...
    public FinanceSyncConsumer(UpSyncService upSyncService, ObjectMapper objectMapper) {
        this.upSyncService = upSyncService;
        this.objectMapper = objectMapper;
    }

    @LocalMqHandler(routingKey = RabbitMQConstant.SYNC_UP_FINANCE_ROUTING_KEY)
    @RabbitListener(queues = RabbitMQConstant.SYNC_UP_FINANCE_QUEUE)
    public void handleSyncMessage(MqCommonMessage<?> message) {
        ...
        log.info("收到财务同步消息: batchUuid={}, tenantId={}, dataCount={}",
                batchUuid,
        ...
        try {
            if (batchDTO == null || batchDTO.getDataList() == null || batchDTO.getDataList().isEmpty()) {
                log.warn("财务同步消息数据为空: batchUuid={}", batchUuid);
                return;
            }
            ...
            log.info("财务批次处理完成: batchUuid={}, 成功={}, 失败={}, 跳过={}",
            ...
            if (result.failCount > 0 && (double) result.failCount / deduplicatedList.size() > 0.3) {
                log.error("财务批次处理失败率过高: batchUuid={}, failRate={}%",
                        batchUuid, (double) result.failCount / deduplicatedList.size() * 100);
            }

        } catch (Exception e) {
            log.error("处理财务同步批次消息异常: batchUuid={}, error={}", batchUuid, e.getMessage(), e);
            throw e;
        }
    }
        ...
        for (SyncDataDTO dataDTO : dataList) {
            ...
            try {
                if (!tryAcquireRecord(recordId)) {
                    result.skipCount++;
                    log.debug("财务记录已处理或正在处理: recordId={}", recordId);
                    continue;
                }
                ...
                try {
                    ...
                    if (success) {
                        ...
                        result.failCount++;
                        log.warn("财务处理失败: recordId={}", recordId);
                    }
                } finally {
                ...
                result.failCount++;
                releaseRecord(recordId);
                log.error("财务同步数据写入失败: recordId={}, tableName={}, error={}",
    ...
    private boolean processSyncData(SyncDataDTO dataDTO, SyncBatchDTO batchDTO) {
        try {
            ...
            log.error("财务数据插入 up_sync 失败: recordId={}, tableName={}, error={}",
