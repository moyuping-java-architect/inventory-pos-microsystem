<template>
  <div class="stock-loss">
    <div class="search-bar">
      <el-form :inline="true" :model="searchForm" class="search-form">
        <el-form-item :label="t('stock.lossNo')">
          <el-input v-model="searchForm.lossNo" :placeholder="t('common.input') + t('stock.lossNo')" clearable />
        </el-form-item>
        <el-form-item>
    ...
    <div class="toolbar">
      <el-button type="primary" icon="Plus" @click="handleAdd">{{ t('stock.addLoss') }}</el-button>
    </div>
    ...
    <div class="table-container">
      <el-table :data="tableData" border stripe v-loading="loading">
        <el-table-column prop="docNo" :label="t('stock.lossNo')" />
    ...
    <el-dialog :title="dialogTitle" v-model="dialogVisible" width="900px">
      <el-form :model="formData" label-width="100px">
        <el-form-item :label="t('stock.lossNo')">
          <el-input v-model="formData.lossNo" disabled />
        </el-form-item>
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="12">
            <el-form-item :label="t('stock.lossDate')">
              <el-date-picker v-model="formData.lossDate" type="date" style="width:100%" :disabled="isViewing" :teleported="false" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-form-item :label="t('stock.lossReason')">
          <el-input v-model="formData.reason" :placeholder="t('common.input') + t('stock.lossReason')" :disabled="isViewing" />
        </el-form-item>
        <el-form-item :label="t('table.items')">
          <div class="items-table">
            <el-table :data="formData.items" border>
              ...
              <el-table-column prop="quantity" :label="t('stock.lossQuantity')" width="110">
                <template #default="scope">
...
const searchForm = reactive({ lossNo: '' })
...
const formData = reactive({
  id: '', lossNo: '', docName: '', warehouseId: '', warehouseName: '', lossDate: '', reason: '', items: [], remark: ''
})
...
const loadList = async () => {
  ...
  try {
    if (activeTab.value === 'formal') {
      ...
      const res = await stockApi.getLossList(params)
      if (res.data) {
      ...
      const params = { docType: 'STOCK_LOSS', pageNum: currentPage.value, pageSize: pageSize.value }
...
const handleReset = () => { searchForm.lossNo = ''; loadList() }

const handleAdd = () => {
  isEdit.value = false
  isViewing.value = false
  dialogTitle.value = t('stock.addLoss')
  formData.id = ''
  formData.lossNo = 'SL' + Date.now()
...
const handleSave = async () => {
  ...
  try {
    ...
    const payload = {
      id: formData.id,
      docType: 'STOCK_LOSS',
      docName: formData.docName || '库存报损' + new Date().toISOString().slice(0,10).replace(/-/g,''),
      creatorId: userId,
      creatorName: userName,
      docNo: formData.lossNo,
      ...
      docDate: formatDateTime(formData.lossDate),
    ...
    if (isEdit.value) {
      ...
      await docApi.submitStockLoss(payload)
