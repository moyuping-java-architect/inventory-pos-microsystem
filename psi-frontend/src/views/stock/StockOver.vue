<template>
  <div class="stock-over">
    <div class="search-bar">
      <el-form :inline="true" :model="searchForm" class="search-form">
        <el-form-item :label="t('stock.overNo')">
          <el-input v-model="searchForm.overNo" :placeholder="t('common.input') + t('stock.overNo')" clearable />
        </el-form-item>
        <el-form-item>
    ...
    <div class="toolbar">
      <el-button type="primary" icon="Plus" @click="handleAdd">{{ t('stock.addOver') }}</el-button>
    </div>
    ...
    <div class="table-container">
      <el-table :data="tableData" border stripe v-loading="loading">
        <el-table-column prop="docNo" :label="t('stock.overNo')" />
    ...
    <el-dialog :title="dialogTitle" v-model="dialogVisible" width="900px">
      <el-form :model="formData" label-width="100px">
        <el-form-item :label="t('stock.overNo')">
          <el-input v-model="formData.overNo" disabled />
        </el-form-item>
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item :label="t('stock.warehouse')" v-if="isViewing && formData.warehouseName">
              <el-input :value="formData.warehouseName" disabled />
            </el-form-item>
            <el-form-item :label="t('stock.warehouse')" v-else required>
          ...
          <el-col :span="12">
            <el-form-item :label="t('stock.overDate')">
              <el-date-picker v-model="formData.overDate" type="date" style="width:100%" :disabled="isViewing" :teleported="false" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-form-item :label="t('stock.overReason')">
          <el-input v-model="formData.reason" :placeholder="t('common.input') + t('stock.overReason')" :disabled="isViewing" />
        </el-form-item>
        <el-form-item :label="t('table.items')">
          <div class="items-table">
            <el-table :data="formData.items" border>
              ...
              <el-table-column prop="quantity" :label="t('stock.overQuantity')" width="110">
                <template #default="scope">
...
import { docApi, stockApi, warehouseApi } from '../../api'
import { ElMessage } from 'element-plus'

const { t } = useI18n()

const searchForm = reactive({ overNo: '' })
...
const formData = reactive({
  id: '', overNo: '', docName: '', warehouseId: '', warehouseName: '', overDate: '', reason: '', items: [], remark: ''
})
...
const loadWarehouses = async () => {
  try {
    warehouses.value = await warehouseApi.getEnabledList()
...
const loadList = async () => {
  ...
  try {
    if (activeTab.value === 'formal') {
      ...
      const res = await stockApi.getOverList(params)
      if (res.data) {
      ...
      const params = { docType: 'STOCK_OVERFLOW', pageNum: currentPage.value, pageSize: pageSize.value }
...
const handleReset = () => { searchForm.overNo = ''; loadList() }

const handleAdd = () => {
  isEdit.value = false
  isViewing.value = false
  dialogTitle.value = t('stock.addOver')
  formData.id = ''
  formData.overNo = 'SOV' + Date.now()
...
const handleSave = async () => {
  ...
  try {
    ...
    const payload = {
      id: formData.id,
      docType: 'STOCK_OVERFLOW',
      docName: formData.docName || '库存报溢' + new Date().toISOString().slice(0,10).replace(/-/g,''),
      creatorId: userId,
      creatorName: userName,
      docNo: formData.overNo,
      ...
      docDate: formatDateTime(formData.overDate),
    ...
    if (isEdit.value) {
      ...
      await docApi.submitStockOverflow(payload)
