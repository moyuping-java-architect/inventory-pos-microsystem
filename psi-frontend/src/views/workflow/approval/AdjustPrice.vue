    ...
    <template #doc-detail="{ detail, formatAmount, docStatusType, getSummaries, t }">
      <div class="doc-section" v-if="detail.doc">
        ...
        <el-descriptions :column="3" border size="small">
          ...
          <el-descriptions-item :label="t('workflow.adjustDate')">{{ detail.doc.docDate || '—' }}</el-descriptions-item>
          <el-descriptions-item :label="t('workflow.itemCount')">{{ detail.doc.itemCount || 0 }}</el-descriptions-item>
          <el-descriptions-item :label="t('workflow.remark')" :span="2">{{ detail.doc.remark || '—' }}</el-descriptions-item>
        </el-descriptions>

        <div class="sub-title">{{ t('workflow.adjustItems') }}{{ detail.doc.itemCount || 0 }}{{ t('workflow.itemsSuffix') }}</div>
        <el-table :data="detail.doc.items || []" border stripe size="small" max-height="400">
          ...
          <el-table-column prop="unitPrice" :label="t('workflow.newPrice')" width="100" align="right" />
