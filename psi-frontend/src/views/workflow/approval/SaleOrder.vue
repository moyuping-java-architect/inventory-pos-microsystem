    ...
    <template #doc-detail="{ detail, formatAmount, docStatusType, getSummaries, t }">
      <div class="doc-section" v-if="detail.doc">
        ...
        <el-descriptions :column="3" border size="small">
          ...
          <el-descriptions-item :label="t('workflow.customer')">{{ detail.doc.partnerName || '—' }}</el-descriptions-item>
          <el-descriptions-item :label="t('workflow.creator')">{{ detail.doc.creatorName || '—' }}</el-descriptions-item>
          <el-descriptions-item :label="t('workflow.docDate')">{{ detail.doc.docDate || '—' }}</el-descriptions-item>
          <el-descriptions-item :label="t('workflow.deliveryDate')">{{ detail.doc.deliveryDate || '—' }}</el-descriptions-item>
          ...
          <el-descriptions-item :label="t('workflow.taxAmount')">{{ detail.doc.taxAmount != null ? '\u00a5' + formatAmount(detail.doc.taxAmount) : '—' }}</el-descriptions-item>
          <el-descriptions-item :label="t('workflow.discountAmount')">{{ detail.doc.discountAmount != null ? '\u00a5' + formatAmount(detail.doc.discountAmount) : '—' }}</el-descriptions-item>
          <el-descriptions-item :label="t('workflow.payAmount')">
            <span class="amount-text pay-amount">{{ detail.doc.payAmount != null ? '\u00a5' + formatAmount(detail.doc.payAmount) : '—' }}</span>
          </el-descriptions-item>
          <el-descriptions-item :label="t('workflow.remark')" :span="2">{{ detail.doc.remark || '—' }}</el-descriptions-item>
        </el-descriptions>

        <div class="sub-title">{{ t('workflow.saleItems') }}{{ detail.doc.itemCount || 0 }}{{ t('workflow.itemsSuffix') }}</div>
        <el-table :data="detail.doc.items || []" border stripe size="small" max-height="400" show-summary :summary-method="getSummaries">
          ...
          <el-table-column prop="quantity" :label="t('workflow.saleQty')" width="100" align="right" />
          <el-table-column prop="unitPrice" :label="t('workflow.salePrice')" width="110" align="right">
            <template #default="{ row }">{{ row.unitPrice != null ? '\u00a5' + formatAmount(row.unitPrice) : '—' }}</template>
          </el-table-column>
          <el-table-column prop="amount" :label="t('workflow.saleAmount')" width="120" align="right">
...
.amount-text.pay-amount { color: #e6a23c; font-size: 15px; }
</style>