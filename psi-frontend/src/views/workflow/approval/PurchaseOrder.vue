  ...
  <ApprovalContainer :title="t('workflow.purchaseOrderTitle')">
    <template #doc-detail="{ detail, formatAmount, docStatusType, getSummaries, t }">
      <div class="doc-section" v-if="detail.doc">
        ...
        <div class="sub-title">{{ t('workflow.purchaseItems') }}{{ detail.doc.itemCount || 0 }}{{ t('workflow.itemsSuffix') }}</div>
        <el-table :data="detail.doc.items || []" border stripe size="small" max-height="400" show-summary :summary-method="getSummaries">
          ...
          <el-table-column prop="quantity" :label="t('workflow.purchaseQty')" width="100" align="right" />
          <el-table-column prop="unitPrice" :label="t('workflow.purchasePrice')" width="110" align="right">
            <template #default="{ row }">{{ row.unitPrice != null ? '\u00a5' + formatAmount(row.unitPrice) : '—' }}</template>
          </el-table-column>
          <el-table-column prop="amount" :label="t('workflow.purchaseAmount')" width="120" align="right">
