      ...
      <el-form :model="ruleForm" label-width="100px">
        ...
        <el-form-item :label="t('workflowSetting.applyProcess')" required>
          <el-select v-model="ruleForm.processCode" style="width: 100%" :placeholder="t('common.select') + t('workflowSetting.applyProcess')">
            ...
            <el-option v-for="def in processDefinitions" :key="def.processKey" :value="def.processKey" :label="def.processName" />
          </el-select>
        </el-form-item>
...
const fetchProcessDefinitions = async () => {
  try {
    ...
    if (res.code === 200) {
      ...
      ElMessage.error(res.message || '查询流程定义失败')
    }
  } catch (error) {
    console.error('查询流程定义失败:', error)
    ElMessage.error('查询流程定义失败')
  }
}
