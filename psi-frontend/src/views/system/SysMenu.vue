      ...
      <el-table :data="tableData" border>
        ...
        <el-table-column prop="menuType" label="菜单类型" width="100">
          <template #default="scope">
            ...
            <span v-else>{{ getMenuTypeName(scope.row.menuType) }}</span>
          </template>
        </el-table-column>
...
const loadMenuList = async () => {
  try {
    ...
    const response = await request.get('/psi/admin/menu/list', { params })
...
const saveAll = async () => {
  ...
  const invalidRows = editableRows.filter(row => !row.menuName.trim())
  if (invalidRows.length > 0) {
    alert(`有 ${invalidRows.length} 行数据的菜单名称为空，请填写完整`)
    return
  }
  ...
  try {
    await request.post('/psi/admin/menu/batch', saveData)
...
const toggleStatus = async (row) => {
  ...
  try {
    await request.put(`/psi/admin/menu/${row.id}/status`, {}, {
...
const handleDelete = async (row) => {
  if (!confirm(`确定要删除菜单 "${row.menuName}" 吗？`)) return
  
  try {
    await request.delete(`/psi/admin/menu/${row.id}`)
    alert('删除成功')
    loadMenuList()
  } catch (error) {
    alert('删除成功（模拟）')
