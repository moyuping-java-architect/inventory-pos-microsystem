      ...
      <div class="search-form">
        <el-form :model="searchForm" inline>
          <el-form-item label="字典标签">
            <el-input v-model="searchForm.dictLabel" placeholder="请输入字典标签" />
          </el-form-item>
          <el-form-item label="字典值">
            <el-input v-model="searchForm.dictValue" placeholder="请输入字典值" />
          </el-form-item>
...
const loadDictDataList = async () => {
  try {
    ...
    const response = await request.get('/psi/admin/dict-data/list', { params })
...
const saveAll = async () => {
  ...
  const invalidTypeRows = editableRows.filter(row => !row.dictType.trim())
  if (invalidTypeRows.length > 0) {
    alert(`有 ${invalidTypeRows.length} 行数据的字典类型为空，请填写完整`)
    return
  }
  
  const invalidLabelRows = editableRows.filter(row => !row.dictLabel.trim())
  if (invalidLabelRows.length > 0) {
    alert(`有 ${invalidLabelRows.length} 行数据的字典标签为空，请填写完整`)
    return
  }
  
  const invalidValueRows = editableRows.filter(row => !row.dictValue.trim())
  if (invalidValueRows.length > 0) {
    alert(`有 ${invalidValueRows.length} 行数据的字典值为空，请填写完整`)
    return
  }
  ...
  try {
    await request.post('/psi/admin/dict-data/batch', saveData)
...
const toggleStatus = async (row) => {
  ...
  try {
    await request.put(`/psi/admin/dict-data/${row.id}/status`, {}, {
...
const handleDelete = async (row) => {
  if (!confirm(`确定要删除字典数据 "${row.dictLabel}" 吗？`)) return
  
  try {
    await request.delete(`/psi/admin/dict-data/${row.id}`)
    alert('删除成功')
    loadDictDataList()
  } catch (error) {
    alert('删除成功（模拟）')
