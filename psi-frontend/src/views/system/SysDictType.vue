      ...
      <div class="search-form">
        <el-form :model="searchForm" inline>
          ...
          <el-form-item label="字典编码">
            <el-input v-model="searchForm.dictCode" placeholder="请输入字典编码" />
          </el-form-item>
...
const loadDictTypeList = async () => {
  try {
    ...
    const response = await request.get('/psi/admin/dict-type/list', { params })
...
const saveAll = async () => {
  ...
  const invalidRows = editableRows.filter(row => !row.dictName.trim())
  if (invalidRows.length > 0) {
    alert(`有 ${invalidRows.length} 行数据的字典名称为空，请填写完整`)
    return
  }
  
  const invalidCodeRows = editableRows.filter(row => row._isNew && !row.dictCode.trim())
  if (invalidCodeRows.length > 0) {
    alert(`有 ${invalidCodeRows.length} 行新数据的字典编码为空，请填写完整`)
    return
  }
  ...
  try {
    await request.post('/psi/admin/dict-type/batch', saveData)
...
const toggleStatus = async (row) => {
  ...
  try {
    await request.put(`/psi/admin/dict-type/${row.id}/status`, {}, {
...
const handleDelete = async (row) => {
  if (!confirm(`确定要删除字典类型 "${row.dictName}" 吗？`)) return
  
  try {
    await request.delete(`/psi/admin/dict-type/${row.id}`)
    alert('删除成功')
    loadDictTypeList()
  } catch (error) {
    alert('删除成功（模拟）')
