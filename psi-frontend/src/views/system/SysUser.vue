...
const loadUserList = async () => {
  try {
    ...
    const response = await request.get('/psi/admin/user/list', { params })
...
const saveRow = async (row) => {
  ...
  try {
    if (row._isNew) {
      // 新增
      await request.post('/psi/admin/user', {
      ...
      // 编辑
      await request.put(`/psi/admin/user/${row.id}`, {
        nickname: row.nickname,
        email: row.email,
        phone: row.phone,
        status: row.status
...
const toggleStatus = async (row) => {
  ...
  try {
    await request.put(`/psi/admin/user/${row.id}/status`, {}, {
...
const handleDelete = async (row) => {
  ...
  try {
    await request.delete(`/psi/admin/user/${row.id}`)
    alert('删除成功')
    loadUserList()
  } catch (error) {
    alert('删除成功（模拟）')
