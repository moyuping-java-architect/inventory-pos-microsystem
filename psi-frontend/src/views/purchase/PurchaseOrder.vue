...
const loadSuppliers = async () => {
  try {
    const res = await purchaseApi.getSupplierList({ pageNum: 1, pageSize: 100 })
    suppliers.value = (res.list || []).map(s => ({ id: s.id, name: s.supplierName, code: s.supplierCode }))
  } catch (e) {
...
const handleSave = async () => {
  ...
  try {
    ...
    const payload = {
      ...
      docDate: formData.orderDate ? new Date(formData.orderDate).toISOString().substring(0, 19) : null,
      deliveryDate: formData.deliveryDate ? new Date(formData.deliveryDate).toISOString().substring(0, 19) : null,
...
onMounted(() => { loadSuppliers(); loadList() })
</script>
