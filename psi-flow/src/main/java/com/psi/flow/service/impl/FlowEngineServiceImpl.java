...
public class FlowEngineServiceImpl implements FlowEngineService {
    ...
    public WfProcessInstance startProcess(ProcessStartDTO startDTO) {
        ...
        processInstance.setTenantId(getCurrentTenantId());
        // 存储流程变量（如采购金额amount等）
        ...
        bizRelation.setTenantId(getCurrentTenantId());
        processInstanceBizMapper.insert(bizRelation);

        // 3.1 如果绑定了业务单据，先提交单据（草稿/已驳回 → 已提交），便于后续审批推进
        if (startDTO.getBizId() != null && !startDTO.getBizId().isEmpty()) {
            try {
                DocResponse doc = docService.findByDocNo(startDTO.getBizId());
                if (doc != null) {
                    DocStatus docStatus = DocStatus.fromValue(doc.getStatus() != null ? doc.getStatus() : 0);
                    if (docStatus.canSubmit()) {
                        docService.submit(doc.getId());
                        log.info("发起流程：单据已提交: docNo={}, beforeStatus={}", doc.getDocNo(), docStatus);
                    }
                }
            } catch (Exception e) {
                log.warn("发起流程：提交单据失败，继续启动流程: bizId={}, error={}", startDTO.getBizId(), e.getMessage());
            }
        }
    ...
    private void createCcLog(String processInstanceId, WfProcessNode node) {
        ...
        ccLog.setDelFlag(0);
        ccLog.setTenantId(getCurrentTenantId());
    ...
    private DocFeignResponse updateDocStatus(WfProcessInstance processInstance) {
        try {
            ...
            // 按当前单据状态智能推进到 COMPLETED：DRAFT/REJECTED→SUBMIT→APPROVE→EXECUTE→COMPLETED
            int statusVal = doc.getStatus() != null ? doc.getStatus() : DocStatus.DRAFT.getValue();
            DocStatus cur = DocStatus.fromValue(statusVal);
            Long docId = doc.getId();
            String docNo = doc.getDocNo();
            log.info("流程完成：推进单据状态: docNo={}, currentStatus={}", docNo, cur);

            if (cur == DocStatus.DRAFT || cur == DocStatus.REJECTED) {
                if (cur.canSubmit()) {
                    doc = docService.submit(docId);
                    cur = DocStatus.fromValue(doc.getStatus());
                    log.debug("  → submit 完成: status={}", cur);
                }
            }
            if (cur == DocStatus.SUBMITTED || cur == DocStatus.APPROVING) {
                if (cur.canApprove()) {
                    doc = docService.approve(docId, "SYSTEM", "系统审批");
                    cur = DocStatus.fromValue(doc.getStatus());
                    log.debug("  → approve 完成: status={}", cur);
                }
            }
            if (cur == DocStatus.APPROVED) {
                if (cur.canExecute()) {
                    try {
                        doc = docService.execute(docId);
                        cur = DocStatus.fromValue(doc.getStatus());
                        log.debug("  → execute 完成: status={}", cur);
                    } catch (Exception ignore) {
                        // 某些单据类型可能没有 execute 动作，忽略继续
                        log.debug("  → execute 跳过: {}", ignore.getMessage());
                    }
                }
            }
            if (cur != DocStatus.COMPLETED && cur != DocStatus.CANCELLED) {
                doc = docService.complete(docId);
                log.debug("  → complete 完成: status={}", DocStatus.fromValue(doc.getStatus()));
            }
