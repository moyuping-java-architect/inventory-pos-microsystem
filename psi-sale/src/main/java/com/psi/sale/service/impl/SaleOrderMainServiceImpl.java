...
public class SaleOrderMainServiceImpl extends ServiceImpl<SaleOrderMainMapper, SaleOrderMainEntity> implements SaleOrderMainService {
    ...
    public SaleOrderMainServiceImpl(SaleOrderItemService saleOrderItemService, MqMessageFacade mqMessageFacade, JdbcTemplate jdbcTemplate, BusinessEventPublisher businessEventPublisher) {
    ...
    public CommonResult<SaleOrderMainDTO> save(SaleOrderSaveDTO saveDTO) {
        ...
        // 提交送审：发布标准业务事件（客户旅程引擎消费）
        if (entity.getCustomerId() != null) {
            businessEventPublisher.publish(
                    BusinessEvent.of("SALE.ORDER_SUBMITTED", BusinessEvent.SUBJECT_CUSTOMER, entity.getCustomerId())
                            .bizKey(entity.getOrderNo())
                            .put("orderNo", entity.getOrderNo())
                            .put("totalAmount", entity.getTotalAmount()));
        }
    ...
    public CommonResult<Void> audit(Long id, Integer auditStatus) {
        ...
        // 审批结果：发布标准业务事件（客户旅程引擎消费）
        if (entity.getCustomerId() != null) {
            String eventCode = auditStatus == 1 ? "SALE.ORDER_APPROVED" : "SALE.ORDER_REJECTED";
            businessEventPublisher.publish(
                    BusinessEvent.of(eventCode, BusinessEvent.SUBJECT_CUSTOMER, entity.getCustomerId())
                            .bizKey(entity.getOrderNo())
                            .put("orderNo", entity.getOrderNo())
    ...
    private BigDecimal getConversionRate(String skuCode, String unitCode, BigDecimal itemRate) {
        ...
        try {
            BigDecimal rate = jdbcTemplate.queryForObject(
                    "SELECT conversion_rate FROM goods_sku_sale_unit WHERE sku_code = ? AND symbol = ? LIMIT 1",
