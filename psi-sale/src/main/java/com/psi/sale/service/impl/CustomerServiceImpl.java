...
public class CustomerServiceImpl extends ServiceImpl<CustomerMapper, CustomerEntity> implements CustomerService {
    ...
    public CommonResult<CustomerDTO> save(CustomerSaveDTO saveDTO) {
        ...
        // 发送标准业务事件（客户旅程引擎消费，本模块不关心下游怎么用）
        businessEventPublisher.publish(
                BusinessEvent.of("CUSTOMER.CREATED", BusinessEvent.SUBJECT_CUSTOMER, entity.getId())
                        .bizKey(entity.getCustomerCode())
                        .put("customerName", entity.getCustomerName())
                        .put("customerCode", entity.getCustomerCode()));
        return CommonResult.success(BeanUtils.convert(entity, CustomerDTO.class));
    }
    ...
    public CommonResult<Void> updateStatus(Long id, Integer status) {
        ...
        // 发送标准业务事件
        businessEventPublisher.publish(
                BusinessEvent.of("CUSTOMER.STATUS_CHANGED", BusinessEvent.SUBJECT_CUSTOMER, entity.getId())
