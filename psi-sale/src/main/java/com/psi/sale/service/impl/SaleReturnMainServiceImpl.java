...
public class SaleReturnMainServiceImpl extends ServiceImpl<SaleReturnMainMapper, SaleReturnMainEntity> implements SaleReturnMainService {
    ...
    public SaleReturnMainServiceImpl(SaleReturnItemService saleReturnItemService, MqMessageFacade mqMessageFacade, JdbcTemplate jdbcTemplate, BusinessEventPublisher businessEventPublisher) {
    ...
    public CommonResult<Void> audit(Long id, Integer auditStatus) {
        ...
        // 销售退货：发布标准业务事件（客户旅程引擎消费）
        if (auditStatus == 1 && entity.getCustomerId() != null) {
            businessEventPublisher.publish(
                    BusinessEvent.of("SALE.ORDER_RETURNED", BusinessEvent.SUBJECT_CUSTOMER, entity.getCustomerId())
                            .bizKey(entity.getReturnNo())
                            .put("returnNo", entity.getReturnNo())
    ...
    private void sendStockIncreaseMessage(SaleReturnMainEntity entity) {
        try {
            ...
            for (SaleReturnItemEntity item : items) {
                ...
                itemMap.put("goodsSpec", item.getGoodsSpec());
