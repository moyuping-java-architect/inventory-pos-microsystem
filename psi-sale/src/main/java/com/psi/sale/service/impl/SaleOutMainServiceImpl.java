...
public class SaleOutMainServiceImpl extends ServiceImpl<SaleOutMainMapper, SaleOutMainEntity> implements SaleOutMainService {
    ...
    public SaleOutMainServiceImpl(SaleOutItemService saleOutItemService, MqMessageFacade mqMessageFacade, JdbcTemplate jdbcTemplate, BusinessEventPublisher businessEventPublisher) {
    ...
    public CommonResult<Void> audit(Long id, Integer auditStatus) {
        ...
        // 出库发货：发布标准业务事件（客户旅程引擎消费）
        if (auditStatus == 2 && entity.getCustomerId() != null) {
            businessEventPublisher.publish(
                    BusinessEvent.of("SALE.ORDER_SHIPPED", BusinessEvent.SUBJECT_CUSTOMER, entity.getCustomerId())
                            .bizKey(entity.getOutNo())
                            .put("outNo", entity.getOutNo())
