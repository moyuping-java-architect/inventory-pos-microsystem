...
public class RabbitMQConstant {
     ...
     * 业务事件总线交换机。
     * <p>
     * 所有与客户相关的业务动作统一发到这里，由客户旅程引擎单点消费。
     * 业务模块只管发事件，不需要知道旅程、触点、规则的存在。
     */
    public static final String BUSINESS_EVENT_EXCHANGE = "psi.business.event.exchange";

    /**
     * 业务事件总线队列
     */
    public static final String BUSINESS_EVENT_QUEUE = "psi.business.event.queue";

    /**
     * 业务事件总线路由键。
     * <p>
     * 所有业务事件共用这一个路由键，具体是哪个事件由消息体内的
     * eventCode 字段区分（取值见 business_event_dict 表）。
     * 这样新增业务事件不需要新增路由键，也不需要改动分发器。
     */
    public static final String BUSINESS_EVENT_ROUTING_KEY = "psi.business.event";
