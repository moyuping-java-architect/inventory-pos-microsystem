...
@Slf4j
@Component
public class StockTransferStrategy implements ProcessCompletedStrategy {

    private static final Set<String> SUPPORTED_TYPES = Set.of("STOCK_TRANSFER");

    private final StockTransferMainService stockTransferMainService;

    public StockTransferStrategy(StockTransferMainService stockTransferMainService) {
    ...
    public void handleProcessCompleted(WfProcessInstance processInstance, String bizId) {
        log.info("调拨单审批通过，开始生成正式数据: processInstanceId={}, bizId={}", processInstance.getId(), bizId);
        try {
            Long id = Long.parseLong(bizId);
            stockTransferMainService.audit(id, 2);
            log.info("调拨单审核成功: bizId={}", bizId);
        } catch (Exception e) {
            log.error("调拨单审核失败: bizId={}, error={}", bizId, e.getMessage(), e);
            throw new RuntimeException("调拨单审核失败: " + e.getMessage(), e);
        }
    }
}
