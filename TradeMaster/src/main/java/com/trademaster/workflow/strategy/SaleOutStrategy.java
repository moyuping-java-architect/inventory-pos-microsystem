...
@Slf4j
@Component
public class SaleOutStrategy implements ProcessCompletedStrategy {

    private static final Set<String> SUPPORTED_TYPES = Set.of("SALE_OUT");

    private final SaleOutMainService saleOutMainService;

    public SaleOutStrategy(SaleOutMainService saleOutMainService) {
    ...
    public void handleProcessCompleted(WfProcessInstance processInstance, String bizId) {
        log.info("销售出库审批通过，开始生成正式数据: processInstanceId={}, bizId={}", processInstance.getId(), bizId);
        try {
            Long id = Long.parseLong(bizId);
            saleOutMainService.audit(id, 2);
            log.info("销售出库单审核成功: bizId={}", bizId);
        } catch (Exception e) {
            log.error("销售出库单审核失败: bizId={}, error={}", bizId, e.getMessage(), e);
            throw new RuntimeException("销售出库单审核失败: " + e.getMessage(), e);
        }
    }
}
