...
@Slf4j
@Component
public class StockLossStrategy implements ProcessCompletedStrategy {

    private static final Set<String> SUPPORTED_TYPES = Set.of("STOCK_LOSS");

    private final StockLossMainService stockLossMainService;

    public StockLossStrategy(StockLossMainService stockLossMainService) {
    ...
    public void handleProcessCompleted(WfProcessInstance processInstance, String bizId) {
        log.info("报损单审批通过，开始生成正式数据: processInstanceId={}, bizId={}", processInstance.getId(), bizId);
        try {
            Long id = Long.parseLong(bizId);
            stockLossMainService.audit(id, 2);
            log.info("报损单审核成功: bizId={}", bizId);
        } catch (Exception e) {
            log.error("报损单审核失败: bizId={}, error={}", bizId, e.getMessage(), e);
            throw new RuntimeException("报损单审核失败: " + e.getMessage(), e);
        }
    }
}
